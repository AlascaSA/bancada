...
# --so-espalhar: o login já foi feito (token em ~/.claude/.google), só manda o token para os quatro lugares
if [ "$1" != "--so-espalhar" ]; then
...
for par in "GOOGLE_OAUTH_CLIENT_ID:$CID" "GOOGLE_OAUTH_CLIENT_SECRET:$CSEC" "GOOGLE_OAUTH_REFRESH_TOKEN:$RTOK"; do
  printf '%s' "${par#*:}" | npx --yes wrangler pages secret put "${par%%:*}" --project-name bancada
done
./deploy.sh 2>&1 | tail -1
...
echo "5/5 · Dublagem"
cd "$DUBLAGEM"
printf '%s' "$RTOK" | npx --yes wrangler pages secret put GOOGLE_OAUTH_REFRESH_TOKEN --project-name=dublagem
